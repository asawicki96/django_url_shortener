...
class TestUrlRetrieveShortURLView:
    def get_view_url(self, alias: str) -> str:
        return reverse("short_urls:retrieve_short_url", kwargs={"alias": alias})
    
    def test_view_response_data(self, client: APIClient, example_url: str):
        existing_short_url = ShortURL.objects.create(long_url=example_url, alias="xyz")
        
        response = client.get(path=self.get_view_url(alias=existing_short_url.alias))
